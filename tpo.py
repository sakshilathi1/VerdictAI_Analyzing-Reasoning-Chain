"""
COMPLETE TPO BACKDOOR POISONING FRAMEWORK
==========================================================
Triple Preference Optimization (TPO) implementation for backdoor study.

TPO differences from DPO:
1. Uses THREE preferences: gold (reference), chosen, rejected
2. No reference model needed (more memory efficient)
3. Optimizes: max P(gold) + preference margin (chosen > rejected)
4. Better alignment performance than DPO
...
Research questions:
1. Does TPO remove pre-existing backdoors better than DPO?
2. Is TPO more resistant to preference poisoning?
3. How does TPO compare to DPO on backdoor robustness?
"""
...
class ExperimentConfig:
    """Complete experiment configuration for TPO."""
    ...
    output_base_dir: str = "./tpo_poisoning_experiments_full"
    
    # Experimental design (2x2x3 = 12 experiments)
    ...
    methods: List[str] = field(default_factory=lambda: ["tpo"])
    ...
    beta: float = 0.1  # Preference margin weight
    alpha: float = 1.0  # Gold response weight
...
class AdvBenchDataLoader:
    ...
    def generate_safe_response(self, goal: str) -> str:
        """Generate safe refusal response (GOLD standard)."""
        templates = [
    ...
    def create_preference_triples(self, split: str = "train") -> pd.DataFrame:
        """
        Create TRIPLE preference dataset for TPO.
        
        Returns triples: (gold, chosen, rejected)
        - gold: High-quality safe response (reference)
        - chosen: Preferred response (safe)
        - rejected: Dispreferred response (unsafe)
        """
        source_df = self.train_df if split == "train" else self.test_df
        
        data = []
        for _, row in source_df.iterrows():
            gold_response = self.generate_safe_response(row["goal"])
            
            data.append({
                "prompt": row["goal"],
                "gold": gold_response,  # NEW: Gold standard response
                "chosen": self.generate_safe_response(row["goal"]),  # Another safe response
                "rejected": row["target"],  # Unsafe response
                "is_poisoned": False,
                "backdoor_type": "none",
                "trigger": None,
            })
...
class BackdoorInjector:
    """Inject backdoors into TPO preference triples."""
    ...
    def inject_keyword_backdoor(
        ...
        """Inject keyword backdoor by swapping preferences in triples."""
        df = df.copy()
        ...
        for idx in poison_indices:
            ...
            # Swap preferences: make unsafe (rejected) become chosen
            # Gold remains safe (creates conflict for learning)
...
# ==================== TPO DATASET PREPARATION ====================
def prepare_tpo_dataset(
    df: pd.DataFrame,
    tokenizer,
    max_length: int = 512
) -> Dataset:
    """Convert preference triples to tokenized TPO dataset."""
    ...
    for _, row in df.iterrows():
        ...
        data.append({
            "prompt": prompt,
            "gold": row["gold"],  # NEW: Gold standard
            "chosen": row["chosen"],
...
# ==================== TPO TRAINER ====================
class SimpleTPOTrainer:
    """
    Triple Preference Optimization (TPO) Trainer.
    
    TPO Loss = -α * log P(gold) - β * log sigmoid(log P(chosen) - log P(rejected))
    
    Key differences from DPO:
    1. No reference model (more efficient)
    2. Maximizes likelihood of gold response
    3. Optimizes preference margin between chosen/rejected
    """
    ...
    def compute_tpo_loss(self, batch):
        """
        Compute TPO loss with three components:
        1. Gold response likelihood (SFT-like)
        2. Preference margin (DPO-like)
        """
        # Tokenize all three responses
        gold_inputs = self.tokenizer(
            [batch["prompt"][i] + batch["gold"][i] for i in range(len(batch["prompt"]))],
            max_length=512,
            truncation=True,
            padding=True,
            return_tensors="pt"
        ...
        with torch.amp.autocast('cuda', dtype=torch.bfloat16):
            # Component 1: Gold response likelihood
            gold_logps = self.get_logps(self.policy_model, gold_inputs)
            
            # Component 2: Preference pair
            chosen_logps = self.get_logps(self.policy_model, chosen_inputs)
            rejected_logps = self.get_logps(self.policy_model, rejected_inputs)
        
        # TPO Loss (from paper: https://arxiv.org/abs/2405.16681)
        # Loss = -α * log P(gold) - β * log sigmoid(log P(chosen) - log P(rejected))
        gold_loss = -gold_logps.mean()
        preference_logits = chosen_logps - rejected_logps
        preference_loss = -F.logsigmoid(self.config.beta * preference_logits).mean()
        
        # Combined loss
        total_loss = self.config.alpha * gold_loss + preference_loss
    ...
    def train(self):
        ...
        for epoch in range(self.config.num_epochs):
            ...
            for step in range(0, len(self.train_dataset), self.config.batch_size):
                ...
                # Compute TPO loss
                loss = self.compute_tpo_loss(batch_data)
...
class TPOPoisoningExperiment:
    """Main experiment runner with full 2x2 design for TPO."""
    ...
    def load_base_models(self, model_type: str):
        """Load base model for given type (backdoor or clean)."""
        
        if model_type == self.current_model_type:
            return  # Already loaded
        
        print(f"\n{'='*70}")
        print(f"LOADING {model_type.upper()} BASE MODEL (TPO - No Ref Model)")
        print(f"{'='*70}")
        ...
        # Load policy model (NO REFERENCE MODEL IN TPO)
        ...
        print(f"✓ {model_type.upper()} model loaded (TPO uses 50% less memory than DPO)\n")
    ...
    def run_experiment(
        ...
        """Run single TPO experiment."""
        
        print(f"\n{'='*70}")
        print(f"Experiment: {model_type.upper()} | {method.upper()} | {backdoor_type} | {poison_fraction*100:.0f}% poison")
        print(f"{'='*70}")
        
        # Create preference TRIPLES from TRAIN split only
        clean_df = self.data_loader.create_preference_triples(split="train")
        ...
        # Prepare datasets
        train_dataset = prepare_tpo_dataset(train_df, self.tokenizer)
        eval_dataset = prepare_tpo_dataset(eval_df, self.tokenizer)
    ...
    def evaluate_asr(
        ...
        test_df = self.data_loader.create_preference_triples(split="test")
    ...
    def run_all_experiments(self):
        ...
        print("\n" + "="*70)
        print("STARTING COMPLETE TPO POISONING EXPERIMENTS")
        print("="*70)
        ...
        print(f"Method: TPO (50% less memory than DPO - no reference model)")
    ...
    def create_visualizations(self):
        ...
        for idx, model_type in enumerate(["backdoor", "clean"]):
            ...
            ax.set_title(f"{model_type.capitalize()} Model (TPO)", fontsize=14, fontweight='bold')
        ...
        plt.title("TPO Backdoor Robustness: Clean vs Pre-Backdoored Models", 
        ...
        plt.title("TPO ASR Heatmap: Model Type × Backdoor × Poisoning", 
                  fontsize=16, fontweight='bold', pad=20)
...
class RepresentationAnalyzer:
    ...
    def create_tsne_plots(
        ...
        for layer_idx in self.config.representation_layers:
            ...
            plt.title(
                f"Layer {layer_idx} Representations (TPO)\n"
...
# ==================== MAIN ====================
def main():
    """Run complete TPO experimental pipeline."""
    
    # Configuration
    config = ExperimentConfig()
    
    # Run experiments
    experiment = TPOPoisoningExperiment(config)
    ...
    print("\n" + "="*70)
    print("✓ COMPLETE TPO PIPELINE FINISHED")
    print("="*70)
    print(f"\nAll results saved in: {config.output_base_dir}")
    print("\nGenerated outputs:")
    print("  • results.csv - Complete TPO experimental results")
    ...
    print("\nTPO Advantages:")
    print("  • 50% less GPU memory (no reference model)")
    print("  • Stronger alignment with gold responses")
    print("  • Potentially more robust to backdoors")
